...
class Jogador():
    def __init__(self, x, y):
        ...
        self.direction_enemy_ = 1
        self.inverted_knockback = False
    ...
    def update(self):
        ...
        if self.invencible == False:
            self.inverted_knockback = False
            self.hitByEnemy_ = False
         
        if self.dano and self.hitByEnemy_:
            if self.direction_enemy_ == 1:
                if self.inverted_knockback:
                    dx -= 7
                else:
                    dx += 7
            else:
        ...
        if self.alive and jogador.teleport == False and jogador.cutscene == False and jogador.andar_sozinho == False:
            ...
            if self.index == 1:
                if self.atirando == False:
                    if abs(self.rect.x - jogador.rect.x) < 700 or (jogador.rect.x < 620 and abs(self.rect.x - jogador.rect.x) < 1344):
                        self.bullet = enemy_bullet(self.rect.x, self.rect.y)
                        self.direction_bullet = self.direction
                        jogador.direction_bullet = self.direction_bullet
                        self.atirando = True
    ...
    def draw(self):
        if self.alive and jogador.teleport == False and jogador.cutscene == False and jogador.andar_sozinho == False:
            ...
            if jogador.bullet.rect.colliderect(self.rect.x - scroll[0], self.rect.y - scroll[1], self.width, self.height):
                ...
                jogador.bullet.kill()
                self.vida -= 1
            if abs(self.rect.x - jogador.rect.x) < 100:
                jogador.direction_enemy_ = self.direction
                if self.rect.x > jogador.rect.x:
                    jogador.inverted_knockback = True
